...
void WinD3DInterface::drawCircle(int x, int y, float radius, int delta, Color color)
{
  ...
  verts = new BoyVertex[numVertex];
  int angle = 0, idx = 0;
  while (angle <= 360)
  {
	  verts[idx].x = (float)x + radius*cos(deg2rad(angle));
	  verts[idx].y = (float)y + radius*sin(deg2rad(angle));
	  verts[idx].z = 0;
	  verts[idx].u = 0;
	  verts[idx].v = 0;
  ...
  IDirect3DVertexBuffer9 *vb = createVertexBuffer(numVertex);
	...
	// copy the data into the buffer:
	int byteCount = sizeof(BoyVertex) * numVertex;
...
void WinD3DInterface::drawSphere(WinSphere *sphere, IDirect3DTexture9 *tex)
{
	// make sure beginScene was called:
	assert(mRendering);
  ...
  hr = sphere->mSphereMesh->DrawSubset(0);
  if(FAILED(hr)) return;
...
void WinD3DInterface::drawLineStrip(WinLineStrip *strip)
{
	// make sure beginScene was called:
	assert(mRendering);
...
bool WinD3DInterface::saveTexture(IDirect3DTexture9 *tex, const char *filenameUtf8)
{
	UString filename(filenameUtf8);

	// save the texture:
	HRESULT hr = D3DXSaveTextureToFile(
		filename.wc_str(), // file to save to
		D3DXIFF_PNG, // file format
		tex, // the texture object to save
    NULL); // no palette

	if (hr!=D3D_OK)
	{
		printf("WARNING: D3DXSaveTextureToFile failed for '%s'\n",filename.toUtf8());
		assertSuccess(hr);
	}
