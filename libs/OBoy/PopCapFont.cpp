...
PopCapFont::PopCapFont(ResourceLoader *loader, const std::string &path) : Resource(loader,path)
{
	assert(loader!=NULL);
...
bool PopCapFont::hasChar(wchar_t ch)
{
	return mChars.find(ch)!=mChars.end();
}

void PopCapFont::addChar(wchar_t ch)
{
...
void PopCapFont::setWidth(wchar_t ch, int width)
{
...
void PopCapFont::setSubrect(wchar_t ch, const BoyLib::Rect &subrect)
{
	// make sure this character has been added:
	assert(mChars.find(ch)!=mChars.end());
...
void PopCapFont::setOffset(wchar_t ch, const BoyLib::Vector2 &offset)
{
	// make sure this character has been added:
	assert(mChars.find(ch)!=mChars.end());
...
int PopCapFont::getHeight()
{
	assert(mHeight>0);
	return mHeight*mScale;
}

int PopCapFont::getStringWidth(const Boy::UString &str)
{
	int width = 0;
...
int PopCapFont::getLineSpacing()
{
	return (mLineSpacing>=0 ? mLineSpacing : mHeight) * mScale;
}

float PopCapFont::drawString(Graphics *g, const Boy::UString &str, float scale)
{
	scale *= mScale;
	g->pushTransform();
...
void PopCapFont::loadCharList(Boy::UStringStream &fontStream, std::vector<wchar_t> &charList)
{
	Boy::UString buf;
...
void PopCapFont::loadWidthList(Boy::UStringStream &fontStream, std::vector<int> &widthList)
{
	Boy::UString buf;
...
void PopCapFont::loadRectList(Boy::UStringStream &fontStream, std::vector<BoyLib::Rect> &rectList)
{
	Boy::UString buf;
...
void PopCapFont::loadOffsetList(Boy::UStringStream &fontStream, std::vector<BoyLib::Vector2> &offsetList)
{
	Boy::UString buf;
	int x,y;
...
void PopCapFont::loadKerningPairs(Boy::UStringStream &fontStream, std::vector<Boy::UString> &kerningPairs)
{
	Boy::UString buf;
...
void PopCapFont::loadKerningValues(Boy::UStringStream &fontStream, std::vector<int> &kerningValues)
{
	Boy::UString buf;
...
bool PopCapFont::init(bool includeSounds)
{
	// create the image:
	assert(mImage != NULL);
...
void PopCapFont::destroy(bool includeSounds)
{
	mImage->release();
}

void PopCapFont::setKerning(wchar_t ch1, wchar_t ch2, int kvalue)
{
