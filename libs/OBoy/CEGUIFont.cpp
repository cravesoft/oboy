...
CEGUIFont::CEGUIFont(ResourceLoader *loader, const std::string &path) : Resource(loader,path)
{
	assert(loader!=NULL);
...
bool CEGUIFont::hasChar(wchar_t ch)
{
	return mChars.find(ch)!=mChars.end();
}

void CEGUIFont::addChar(wchar_t ch)
{
...
void CEGUIFont::setWidth(wchar_t ch, int width)
{
...
void CEGUIFont::setSubrect(wchar_t ch, const BoyLib::Rect &subrect)
{
	// make sure this character has been added:
	assert(mChars.find(ch)!=mChars.end());
...
void CEGUIFont::setOffset(wchar_t ch, const BoyLib::Vector2 &offset)
{
	// make sure this character has been added:
	assert(mChars.find(ch)!=mChars.end());
...
int CEGUIFont::getHeight()
{
	assert(mHeight>0);
	return mHeight*mScale;
}

int CEGUIFont::getStringWidth(const Boy::UString &str)
{
	int width = 0;
...
int CEGUIFont::getLineSpacing()
{
	return (mLineSpacing>=0 ? mLineSpacing : mHeight) * mScale;
}

float CEGUIFont::drawString(Graphics *g, const Boy::UString &str, float scale)
{
	scale *= mScale;
	g->pushTransform();
...
void CEGUIFont::loadCharList(Boy::UStringStream &fontStream, std::vector<wchar_t> &charList)
{
	Boy::UString buf;
...
void CEGUIFont::loadWidthList(Boy::UStringStream &fontStream, std::vector<int> &widthList)
{
	Boy::UString buf;
...
void CEGUIFont::loadRectList(Boy::UStringStream &fontStream, std::vector<BoyLib::Rect> &rectList)
{
	Boy::UString buf;
...
void CEGUIFont::loadOffsetList(Boy::UStringStream &fontStream, std::vector<BoyLib::Vector2> &offsetList)
{
	Boy::UString buf;
	int x,y;
...
void CEGUIFont::loadKerningPairs(Boy::UStringStream &fontStream, std::vector<Boy::UString> &kerningPairs)
{
	Boy::UString buf;
...
void CEGUIFont::loadKerningValues(Boy::UStringStream &fontStream, std::vector<int> &kerningValues)
{
	Boy::UString buf;
...
bool CEGUIFont::init(bool includeSounds)
{
	// create the image:
	assert(mImage != NULL);
...
void CEGUIFont::destroy(bool includeSounds)
{
	mImage->release();
}

void CEGUIFont::setKerning(wchar_t ch1, wchar_t ch2, int kvalue)
{
