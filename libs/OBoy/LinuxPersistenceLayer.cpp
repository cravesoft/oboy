...
bool LinuxPersistenceLayer::remove(const std::string &name, bool persist)
{
	// find the name:
...
void LinuxPersistenceLayer::putString(const std::string &name, const std::string &value, bool persist)
{
...
const std::string LinuxPersistenceLayer::getString(const std::string &name)
{
	std::map<std::string,std::string>::iterator iter = mValues.find(name);
	if (iter!=mValues.end())
	{
...
void LinuxPersistenceLayer::load()
{
	BoyFileHandle hFile;
...
void LinuxPersistenceLayer::save()
{
...
void LinuxPersistenceLayer::parse(char *data, int size)
{
	int namePos;
	int nameSize;
	int valuePos;
...
int LinuxPersistenceLayer::readInt(char *data, int *pos)
{
	int value = 0;
...
int LinuxPersistenceLayer::getKeyCount()
{
	return (int)mValues.size();
}

const std::string LinuxPersistenceLayer::getKey(int i)
{
